    ...
    >
      <thead>
        <tr>
          {addInputActive === false ? (
            <th className="font-p font-bold text-black text-xl max-[500px]:text-sm pb-6">
              ID
            </th>
          ) : null}
          <th className="font-p font-bold text-black text-xl max-[500px]:text-sm pb-6">
      ...
      {products.map((product, index) => (
        <tr
          ...
          onClick={() =>
            router.push(`product/${product.id !== 0 ? product.id : null}/`)
          }
        >
          {addInputActive === false ? (
            <td className="text-center pb-4">
              <p className="font-p text-black font-light text-xl max-[500px]:text-base">
                {product.id !== 0 ? product.id : <Loader />}
              </p>
            </td>
          ) : null}
          <td className="text-center pb-4">
            <p className="font-p text-black font-light text-xl max-[500px]:text-base">
              {product.name !== "" ? product.name : <Loader />}
            </p>
          </td>
          <td className="text-center pb-4">
            <p className="font-p text-black font-light text-xl max-[500px]:text-base">
              {product.price.toString() !== "0.00" ? product.price : <Loader />}{" "}
              R$
            </p>
          </td>
        </tr>
