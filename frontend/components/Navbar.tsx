...
function Navbar() {
  ...
  return (
    <nav className="bg-white w-full h-auto flex flex-row justify-end items-center px-6 gap-4 flex-wrap py-5 max-[430px]:w-[350px] pl-20 text-end">
      <h1 className="font-h1 text-black text-2xl max-[300px]:text-xl">FastDash</h1>
      {loader === false ? (
        <h2 className="font-h2 text-xl text-black max-[300px]:text-sm">{t("Welcome") + " " + firstname}</h2>
      ) : (
        <Loader />
      )}
    </nav>
