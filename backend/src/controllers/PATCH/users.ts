...
export class UpdateUserController implements IUpdateController<User> {
  constructor(
    private readonly updateUserRepository: IUpdateRepository<Partial<User>>
  ) {}
  ...
  async handle(
    ...
    try {
      ...
      if (
        httpRequest?.params?.id === null ||
        httpRequest?.params?.id === undefined
      ) {
      ...
      if (
        httpRequest?.permission === null ||
        httpRequest?.permission === undefined
      ) {
        return badRequest("Please specify a permission to update user");
      }

      if (
        httpRequest?.jwt_token === null ||
        httpRequest?.jwt_token === undefined
      ) {
        return badRequest("Please specify the token to update user");
      }

      const token = httpRequest.jwt_token;
      ...
      if (id === null || id === undefined) {
        return badRequest("Invalid JWT token, this user does not exists");
      }

      if (id.toString() !== httpRequest?.params?.id) {
        return badRequest("Invalid JWT token, this user is not");
      }
      ...
      if (
        httpRequest?.permission === "manager" &&
        httpRequest?.body?.role === "admin"
      ) {
        return badPermission(
          "Not authorized, manager does not have all the permissions required"
        );
      }
      // Check if operational level is trying to not required properties
      if (
        httpRequest?.permission === "operational" &&
        ...
        return badPermission(
          "Not authorized, this user has to update only his first name and/or last name"
        );
      }
      ...
      await this.updateUserRepository.updateModel(
